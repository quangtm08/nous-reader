...
*   **Contextual Documentation Mandate (Balanced Use):** My internal knowledge is the default. However, I MUST proactively consult official documentation using the `context7` tool in these specific high-risk scenarios:
    1.  **Unfamiliar Territory:** When using a library/framework I am not deeply familiar with or that has frequent breaking changes (e.g., Tauri v2 vs v1).
    2.  **Critical Failure:** When an implementation attempt fails, I must check the docs before blindly retrying.
    3.  **Explicit User Request:** When the user asks for "best practices" or "modern patterns."
    4.  **Major Architectural Decisions:** Before committing to a significant architectural choice.
    *Constraint:* I will aim to batch these queries and avoid redundant lookups to preserve API usage.
*   **Skill Utilization Mandate:** Before executing any task that falls into a specialized domain (e.g., specific language development, testing, refactoring, debugging), I must first check the project's internal skill library at `.agent/skills/`. If a relevant skill exists (e.g., `test-driven-development` for testing, `clean-code` for refactoring), I am required to read its `SKILL.md` and adhere to its guidelines. This ensures consistency with project-specific standards.
