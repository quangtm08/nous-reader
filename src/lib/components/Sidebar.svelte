...
{#if $isSidebarOpen}
  ...
  <!-- Drawer -->
  <aside 
    class="fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-surface/95 backdrop-blur-xl border-r border-white/5 shadow-2xl"
  ...
  >
    <div class="p-6 flex justify-between items-center mb-4">
      ...
      <button onclick={close} class="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
        <X size={20} />
      </button>
    </div>
    ...
    <nav class="flex-1 px-4 space-y-2">
      ...
      <button class="w-full flex items-center space-x-3 p-3 rounded-xl text-gray-400 hover:bg-white/5 hover:text-white group transition-all">
        <Search size={20} />
    ...
    <div class="p-4 mt-auto border-t border-white/5 space-y-2">
      <button class="w-full flex items-center space-x-3 p-3 rounded-xl text-gray-400 hover:bg-white/5 hover:text-white transition-all">
        <PlusCircle size={20} />
